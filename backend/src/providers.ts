...
function friendlyProviderFailure(provider: ProviderName, error: unknown) {
  ...
  if (/invalid[_ -]?api[_ -]?key|incorrect api key|unauthorized|401/i.test(detail)) {
    return `no pude usar ${provider} porque la API key no es valida. RevÃ­sala en Railway/backend .env o cambia el modelo a Auto/Local mientras tanto.`;
  }
...
function localReply(lastUserMessage: string, memoryContext: string, taskContext: string) {
  ...
  const explainMatch = lower.match(/(?:qu[eÃ©]\s+es|explica|dime sobre|me puedes decir(?:me)?(?: que es| sobre)?)\s+(.{2,80})/i);
  if (explainMatch) {
...
function answerKnownTopic(lower: string) {
  if (/(lenguajes?|idiomas?)\s+de\s+programaci[oÃ³]n|programming languages|lenguajes?\s+mas\s+usados|lenguajes?\s+m[aÃ¡]s\s+usados/i.test(lower)) {
    return [
