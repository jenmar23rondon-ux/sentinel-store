...
app.post("/api/reports/pdf", (req, res, next) => {
  try {
    ...
    doc.moveDown();
    doc.fontSize(10).fillColor("#666").text(`Generated by Aether - ${new Date().toLocaleString()}`);
    doc.moveDown();
...
function shouldUseWebSearch(message: string) {
  ...
  if (/(crea una grafica|grafica|gr�fica|tarea:|pendiente:|recuerda que|recuerda:|envia|enviar|agenda|agendar)/i.test(lower)) return false;
  if (/(internet|web|busca|buscar|noticia|noticias|actual|hoy|ahora|precio|valor|bitcoin|oro|dolar|d�lar|clima|quien|qu� es|que es|c�mo funciona|como funciona|me puedes decir|dime sobre|explica|mejor|recomienda|beneficios|riesgos|concentracion|concentraci�n|productividad|estudiar|aprender|salud|tecnologia|tecnolog�a|programacion|programaci�n|ciberseguridad|ultimas|�ltimas|latest|news|today|current|\?)/i.test(message)) return true;
...
function slugify(value: string) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 80) || "aether-report";
}
